 ...
 *
 * The rebase runs in tests/rebase.e2e.mjs — its own file, because it builds a
 * second fixture site and two suites competing for one event loop made Node's
 * runner abort tests that had not failed.
 *
...
describe('the document editor', {
  ...
  test('the work survives a reload, and comes back as the same proposal', async () => {
    ...
    await page.waitForFunction(() => {
      ...
      // Generous on purpose. This asserts that a draft survives a reload, not
      // how fast it does: three suites build fixture sites and drive browsers
      // concurrently, and a correctness assertion that fails on a busy machine
